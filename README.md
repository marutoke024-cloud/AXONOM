...
- **Block-style modeling** — 矩形・円/楕円・L字・自由多角形・壁（直線）、頂点ドラッグ変形、厚みの押し出し、Boolean演算（接合 / 型抜き / 抽出 / 除外 / 切り取り）、左右・上下反転（アイコンは記号の向きも反転）、透明塗り（ワイヤーフレーム表示）
- **Layering** — 図形ごとの「高さ位置」スライダーで床から浮かせて積み上げ可能。重なり順は「Objects」パネルでドラッグ並べ替え（PPTの「オブジェクトの選択と表示」風）。3D視点では奥の物体が手前の不透明な物体に正しく隠れる
